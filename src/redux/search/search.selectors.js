...
const selectRecommended = createSelector(
  [selectSearch],
  search => search.products.recommended
);
const selectPopular = createSelector(
  [selectSearch],
  search => search.products.popular
);
...
export const selectProductsOrganized = createSelector(
  [selectProducts],
  products =>
    products
            ...
            if (currentProductsStoreKeys.length > 0) {
              ...
              currentProductsStoreKeys.map(ecommerceSite => {
                ...
                let currentProductData =
                  currentProductsStore[ecommerceSite].data;
...
export const selectRecommendedProductsCount = createSelector(
  [selectRecommended],
  products => (Object.keys(products).length > 0 ? true : false)
);

//* Select n for number of products found in POPULAR
export const selectPopularProductsCount = createSelector(
  [selectPopular],
  products => (Object.keys(products).length > 0 ? true : false)
);
